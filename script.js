...
fecha.innerHTML = FECHA.toLocaleDateString('es-ES', {weekday:'long', month: 'short', day: 'numeric'}); //Muestra la fecha en el html

//Funcion agregar tarea con el estilo predefinido 

function agregarTarea (tarea, id, realizado, eliminado){ //Plantilla para ir creando tareas 

    if (eliminado){ //Si la tarea esta eliminada, no se agrega
        return;
    }
...
// funcion de tarea realizada
    ...
    function tareaRealizada(element){ // lee el elemento por que dependiendo de donde se haga click, se hara una u otra, esto viene de la funcion de seleccionar tareas
        ...
        LIST[element.id].realizado = LIST[element.id].realizado ? false : true; //Cambia el estado de la tarea de primero sera false 
    }
...
function cargarLista(DATA){
    DATA.forEach(function(i){ //Recorre el array y los manda a llamar
